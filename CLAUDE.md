...
- **月額コスト概算表示**: ローン返済額+管理費+修繕積立金の合計月額を表示（デフォルト: 金利0.5%, 35年返済）
- **住宅ローンシミュレーション**: 頭金・金利・返済期間をスライダーで調整してリアルタイム計算（詳細ページ、折りたたみUI）
- **お気に入り機能**: 物件を☆ボタンでブックマーク、ポップアップでサイト横断一覧管理（chrome.storage.local使用）
...
詳細ページでローン返済額+管理費+修繕積立金の月額合計を表示。

- デフォルト: 金利0.5%、35年返済、頭金0円
- 管理費・修繕積立金が取得できない場合は注記付きで表示

**関連関数:** `calculateMonthlyLoanPayment()`, `extractManagementAndRepairFees()`, `displayMonthlyCost()`
...
**関連関数:** `displayLoanSimulation()`, `createSimInputRow()`
...
- 保存データ: `{ url, name, price, tsubotanka, site, area, addedAt }`
- 一覧ページ・詳細ページの坪単価表示横に☆/★ボタン
- ポップアップUI（popup.html）でお気に入り一覧表示、サイト別フィルタ、削除

**権限:** `storage`（お気に入りデータのブラウザ内保存用。データは外部送信しない）

**関連ファイル:** popup.html, popup.js, popup.css
**関連関数（content.js）:** `loadFavoriteUrls()`, `addFavorite()`, `removeFavorite()`, `createFavoriteButton()`
