...
- **重複物件のグルーピング**: 同じ物件の複数掲載を自動でまとめ、最安値と掲載社を一覧表示
...
**取得データ:**
- 基本情報: サイト名、物件名、住所、価格、専有面積、坪単価、平米単価
- 詳細情報（詳細ページから取得）: 階数、向き、建物階数、管理費、修繕積立金、総戸数、構造、駐車場、築年月
- 重複物件情報: 掲載数、不動産会社リスト、全URL

**処理フロー:**
1. 一覧ページの全物件を収集
2. 各物件の詳細ページにアクセスして追加情報を取得（2秒間隔、5件ごとに3秒待機）
3. 住所・面積・階数で重複物件をグルーピング
4. CSV生成（BOM付きUTF-8でExcel対応）
5. ファイル名: `物件一覧_SUUMO_20251004_123456.csv`

**重複物件のグルーピング:**
- 判定基準: 住所 + 専有面積(±0.1㎡) + 階数が同じ
- 代表物件: 最安値の物件を採用
- 不動産会社: 全社名をカンマ区切りで記録
- URL: 代表物件のURLと全URLを記録
