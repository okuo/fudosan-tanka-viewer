...
お気に入り登録済み物件を再訪した時、またはバックグラウンド再チェック時に現在価格と保存済み価格を比較して値下がり/値上がりを表示する。

- 一覧ページ・詳細ページの単価表示内に `価格改定検知` バッジを表示
- ポップアップでは直近の値動き、確認日時、価格推移グラフを表示
- 掲載中/掲載終了の可能性/確認失敗を `listingStatus` として保存・表示
- `priceHistory` は最大20件まで保持
- 価格が変わらない場合も `lastCheckedAt` を更新
- 価格改定順ソートは `priceUpdatedAt` を使用
- 自動再チェックは6時間ごとに起動し、24時間以上未確認の物件を最大5件ずつ確認。各取得の間隔は2秒。
...
3. 詳細取得に失敗した物件も一覧情報だけでCSVに含め、`詳細取得ステータス` に結果を出力
4. CSV生成（BOM付きUTF-8でExcel対応）
5. ファイル名: `物件一覧_{SITE_TYPE}_20251004_123456.csv`
